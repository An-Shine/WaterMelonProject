...
public class BallLauncher : MonoBehaviour
{
    ...
    void SpawnNextBall()
    {
        // FruitSpawner를 이용해 랜덤 과일 프리팹을 가져옵니다.
        GameObject ballPrefab = fruitSpawner.GetRandomFruitPrefab();
        if (ballPrefab == null) return;

        // spawnPoint의 위치와 회전값에 맞춰 새로운 공을 생성합니다.
    ...
    void LaunchBall()
    {
        // 물리 효과 스위치를 다시 켭니다 (중력 적용 시작).
        currentBallRigidbody.isKinematic = false;

        // spawnPoint의 정면 방향으로 힘을 가해 발사합니다.
        // ForceMode.Impulse는 순간적으로 강력한 힘을 가하는 방식입니다.
        currentBallRigidbody.AddForce(spawnPoint.forward * launchForce, ForceMode.Impulse);

        // 발사된 공에 대한 참조를 비워서, 다시 발사할 수 없도록 합니다.
        currentBall = null;
        currentBallRigidbody = null;

        // 다음 공을 바로 생성합니다. (원한다면 코루틴으로 딜레이를 줄 수 있습니다)
        SpawnNextBall();
    }
}
